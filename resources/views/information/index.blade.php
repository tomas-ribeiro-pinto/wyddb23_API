                ...
                <div class="inline-block min-w-full py-2 align-middle sm:px-6 lg:px-8 mb-8">
                    <div class="overflow-hidden shadow ring-1 ring-black ring-opacity-5 sm:rounded-lg">
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            @forelse($information_groups as $information)
                                <tr>
                                    <td class="whitespace-wrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{$information->title_pt}}</td>
                                    <td class="w-36">
                                        @if(!auth()->user()->hasRole('notifier'))
                                            <div class="flex">
                                                <div x-data='{ show: false }' class="mr-2">
                                                    ...
                                                    <x-edit-information :information="$information"/>
                                                </div>
                                                @if(auth()->user()->hasRole('admin'))
                                                    <div x-data="{ show: false }" class="mr-2">
                                                        ...
                                                        <x-confirmation-modal :model="$information"/>
                                                    </div>
                                                @endif
                                            </div>
                                        @endif
