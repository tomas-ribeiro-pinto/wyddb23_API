    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4 sm:px-6 lg:px-8">
                <a href="{{ route('edit-prayers') }}" class="text-sm inline-flex items-center hover:underline font-semibold text-green-700 mb-6">
                ...
                <h2 class="text-2xl font-bold">Orações <span class="font-semibold text-2xl text-white leading-tight p-2 bg-black rounded-2xl">Dia {{date('d/m', strtotime($day->day))}}</span></h2>
            </div>
            <div class="overflow-hidden">
                <div x-data='{ show: false }' class="sm:px-6 lg:px-8 mb-8">
                    ...
                    <x-add-prayer :day="$day" :order="$order"/>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            <tr>
                                <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-white sm:pl-6">
                                    <a class="py-2 hover:underline cursor-pointer hover:font-bold">Ordem</a>
                                </th>
                                <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-white sm:pl-6">
                            ...
                            @forelse($prayerDays as $prayer)
                                <tr>
                                    <td class="w-36 whitespace-wrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{$prayer->order_index}}</td>
                                    <td class="whitespace-wrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{$prayer->title_pt}}</td>
                                    <td class="w-24">
                                        <div class="flex">
                                            <div x-data='{ show: false }' class="mr-2">
                                                ...
                                                <x-edit-prayer :prayer="$prayer" :day="$day"/>
                                            </div>
                                            @if(auth()->user()->hasRole('admin'))
                                                <div x-data="{ show: false }" class="mr-2">
                                                    ...
                                                    <x-confirmation-modal :model="$prayer"/>
                                                </div>
                                            @endif
                                        </div>
                                    </td>
