    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="mb-4 sm:px-6 lg:px-8">
                <a href="{{ route('edit-accommodation') }}" class="text-sm inline-flex items-center hover:underline font-semibold text-green-700 mb-6">
                ...
                <h2 class="text-2xl font-bold"><span class="font-semibold text-2xl text-white leading-tight p-2 bg-green-500 rounded-2xl">{{ucwords($location)}}</span></h2>
            </div>
            <div class="overflow-hidden">
                <div x-data='{ show: false }' class="sm:px-6 lg:px-8 mb-8">
                    ...
                    <x-add-accommodation :location="$location"/>
                </div>
                        ...
                        <table class="min-w-full divide-y divide-gray-300">
                            ...
                            <tr>
                                <th scope="col" class="px-3 py-3.5 text-left text-sm font-semibold text-white">
                                    <a class="py-2 hover:underline cursor-pointer hover:font-bold">Centro Acolhimento</a>
                                </th>
                                <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-white sm:pl-6">
                                    <a class="py-2 hover:underline cursor-pointer hover:font-bold">Morada</a>
                                </th>
                                <th scope="col" class="py-3.5 pl-4 pr-3 text-left text-sm font-semibold text-white sm:pl-6">
                                    <a class="py-2 hover:underline cursor-pointer hover:font-bold">Contacto Telefónico</a>
                                </th>
                            ...
                            @forelse($accommodationLocations as $accommodationLocation)
                                <tr>
                                    <td class="whitespace-wrap py-4 pl-4 pr-3 text-sm font-medium text-gray-900 sm:pl-6">{{$accommodationLocation->name}}</td>
                                    <td class="whitespace-wrap px-3 py-4 text-sm text-gray-500">{{$accommodationLocation->address_line1}}<br>{{$accommodationLocation->address_line2}}</td>
                                    <td class="whitespace-wrap px-3 py-4 text-sm text-gray-500">{{$accommodationLocation->contact}}</td>
                                    <td class="w-36">
                                        @if(!auth()->user()->hasRole('notifier'))
                                            <div class="flex">
                                                <div x-data='{ show: false }' class="mr-2">
                                                    ...
                                                    <x-edit-accommodation :accommodationLocation="$accommodationLocation"/>
                                                </div>
                                                @if(auth()->user()->hasRole('admin'))
                                                    <div x-data="{ show: false }" class="mr-2">
                                                        ...
                                                        <x-confirmation-modal :model="$accommodationLocation"/>
                                                    </div>
                                                @endif
                                            </div>
                                        @endif
